    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="flex items-center justify-center gap-3">
          <div className="w-12 h-12 rounded-full bg-[#2f456d] flex items-center justify-center shadow-md">
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Seu assistente de IA especializado em todas as matérias de concursos públicos
        </p>
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 max-h-[400px]">
            {messages.length === 0 && (
              <div className="text-center py-8 space-y-6">
                <div className="w-16 h-16 mx-auto rounded-full bg-[#2f456d]/10 flex items-center justify-center">
                  <Bot className="w-8 h-8 text-[#2f456d]" />
                </div>
                <div className="space-y-2">
                  <p className="font-medium text-slate-700 dark:text-slate-200">
                    Olá, {user?.full_name?.split(' ')[0]}! Sou o E-Tutory.
                  </p>
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    {suggestedQuestions.map((question, idx) => (
                      <button
                        ...
                        className="text-sm px-3 py-2 bg-[#2f456d]/10 text-[#2f456d] dark:text-blue-300 rounded-full hover:bg-[#2f456d]/20 transition-colors border border-[#2f456d]/20"
                      >
                        {question}
                      </button>
                    ))}
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-8 h-8 rounded-full bg-[#2f456d]/10 flex items-center justify-center shrink-0">
            ...
            {isLoading && (
              <div className="flex gap-3 justify-start">
                <div className="w-8 h-8 rounded-full bg-[#2f456d]/10 flex items-center justify-center">
                  <Bot className="w-4 h-4 text-[#2f456d]" />
                </div>
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
            <div className="flex gap-3">
              <Input
                ...
                placeholder="Digite sua dúvida sobre concursos..."
              ...
              <Button
                ...
                className="bg-[#f26836] hover:bg-[#d9561f] text-white px-6"
              >
      ...
      <Card className="bg-[#2f456d]/5 dark:bg-[#2f456d]/20 border-[#2f456d]/20">
        <CardContent className="p-6">
          <div className="flex items-start gap-3">
            <BookOpen className="w-5 h-5 text-[#2f456d] dark:text-blue-400 mt-0.5" />
            <div>
              <h4 className="font-semibold text-[#2f456d] dark:text-blue-200 mb-2">
